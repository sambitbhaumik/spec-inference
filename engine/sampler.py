...
class SamplingConfig:
    ...
    min_p: float = 0.0        # Minimum probability threshold relative to top token (0.0 = disabled)
...
def _apply_min_p(logits: torch.Tensor, min_p: float) -> torch.Tensor:
    ...
    Returns:
        Filtered logits, -inf for tokens below min_p threshold
    """
    if min_p <= 0.0:
        return logits
...
def filter_logits(logits: torch.Tensor, config: SamplingConfig) -> torch.Tensor:
    """Apply top-k, top-p, and min-p filtering sequentially."""
    logits = _apply_top_k(logits, config.top_k)
    logits = _apply_top_p(logits, config.top_p)
    logits = _apply_min_p(logits, config.min_p)
    return logits
